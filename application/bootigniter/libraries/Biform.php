...
class Biform
{
    ...
    public function __construct( array $attrs = array() )
    {
        ...
        if ( $template = $this->_ci->config->item( 'biform_template' ) )
        {
    ...
    {
        if ( in_array( $attributes['type'], $this->_file_fields ) )
        {
    ...
    protected function _compile( array $field_attrs, $is_sub = FALSE )
    {
        ...
        else if ( $type == 'info' )
        {
            ...
            $html .= '<div class="form-info '.$class.'" id="'.$id.'">'
                  ...
                  .  '<p>'.( is_array( $value ) ? implode( '</p><p>', $value ) : $value ).'</p>'
                  .  '</div></div>';
        }
        else
        {
            ...
            switch( $type )
            {
                ...
                case 'spinner':
                    ...
                    $input = form_input( array(
                            ...
                            'class'             => $input_class.' jqui-spinner'
                            ), set_value( $name, $std ), $attr );
                    break;
                    ...
                    if ( $type == 'rangeslider' )
                    {
                        ...
                        $form_input = '<div class="input-group">'
                                    . form_input( array(
                                        ...
                                        'style' => 'width: 50%;',
                                        'class' => $input_class
                                        ), set_value( $name.'_min', $std['min'] ), $attr )
                                    . form_input( array(
                                        'name'  => $name.'_max',
                                        'id'    => $id.'-max',
                                        'type'  => 'number',
                                        'style' => 'width: 50%;',
                                        'class' => $input_class
                                        ), set_value( $name.'_max', $std['max'] ), $attr )
                                    . '</div>';
                    }
                    else
                    {
                ...
                case 'datepicker':
                    ...
                    $input .= form_input( array(
                        ...
                        'type'  => 'text',
                        'id'    => $id,
                        'class' => $input_class.' bs-datepicker'
                        ), set_value( $name, $std ), $attr );
                ...
                case 'file':
                case 'upload':
                    if ( !isset( $allowed_types ) ) $allowed_types = get_conf( 'allowed_types' );
                ...
                case 'checkbox':
                    ...
                    $devide = ( count( $options ) >= 6 ? TRUE : FALSE );
                ...
                default:
                    log_message('error', '#BootIgniter: Biform->Compile ERROR '.$type.' Field type are not supported currently');
                    break;
            }
        ...
        foreach ( $this->_fields as $field )
        {
            if ( $field['type'] == 'subfield' )
            {
                foreach ( $field['fields'] as $sub )
                {
                    $this->set_field_rules(
                        ...
                        $sub['type'],
                        ( isset( $sub['validation'] ) ? $sub['validation'] : '' ),    // Subfield Validation
                        ( isset( $sub['callback'] )   ? $sub['callback']   : '' ) );  // Subfield Callback ( optional )
                }
            }
            else if ( $field['type'] == 'rangeslider' )
            {
                $this->set_field_rules(
                    ...
                    $field['type'],
                    ( isset( $field['validation'] ) ? $field['validation'] : '' ),    // Field Validation
                    ( isset( $field['callback'] )   ? $field['callback']   : '' ) );  // Field Callback ( optional )
